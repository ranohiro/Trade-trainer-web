...
def draw_candlestick(df, title, trade_history=None):
    ...
    if trade_history:
        ...
        # This can be slow if history is huge, but for sim it's fine.
        buy_dates = [t['Date'] for t in trade_history if t['Action'] == 'BUY']
        buy_prices = [t['Price'] for t in trade_history if t['Action'] == 'BUY']

        sell_dates = [t['Date'] for t in trade_history if t['Action'] == 'SELL']
        sell_prices = [t['Price'] for t in trade_history if t['Action'] == 'SELL']

        close_dates = [t['Date'] for t in trade_history if t['Action'] == 'CLOSE']
        close_prices = [t['Price'] for t in trade_history if t['Action'] == 'CLOSE']
